...
df_f1_by_worker <- aggregate(x=df_eval_compelte[,c("judgement")], by=list(df_eval_compelte$workerId), FUN = f1_by_worker);
names(df_f1_by_worker) <- c("workerId", "f1");
message(paste0(capture.output(df_f1_by_worker), collapse = "\n"));

dev.new(width=500, height=500);
par(mar=c(5,5,2,1)+0.1,bg="white"); 
hist(df_f1_by_worker$f1,
     ...
     axes = FALSE);
axis(side = 1, at=seq(0.1,1,0.1));
axis(side = 2, at=seq(0,36,2));

dirNameForPlots <- file.path(getwd(), "plotting-out", format(Sys.time(), "%Y-%m-%d-%H-%M"));
dir.create(dirNameForPlots, recursive = TRUE, showWarnings = FALSE);
dev.copy(png,file.path(dirNameForPlots, paste0("plot-f1-per-worker.png")));
dev.off();
...
write.csv(df_kappa_by_worker, "kappa_by_worker-lenient.csv");
