...
#using the aggregated list to create a dataframe (id, Crowd_Judgement) for TP_C and FP_C
df_eval_c <- df_drvr_kk_s17[,c("id", "Expert_Comparison")];
...
li_eval_id_tp <- df_eval_c_e[df_eval_c_e$Crowd_Judgement=="TPC"&df_eval_c_e$Expert_Judgement=="TPE",][,c("id")];
c_eval_tp <- rep(c("TP"), length(li_eval_id_tp));
df_eval_tp <- data.frame("id"=li_eval_id_tp, "judgement"=c_eval_tp);

li_eval_id_fp <- df_eval_c_e[df_eval_c_e$Crowd_Judgement=="TPC"&df_eval_c_e$Expert_Judgement=="FPE",][,c("id")];
c_eval_fp <- rep(c("FP"), length(li_eval_id_fp));
df_eval_fp <- data.frame("id"=li_eval_id_fp, "judgement"=c_eval_fp);
...
message("--- LENIENT EVALUATION ---\n");
precision <- fun_precison(tp=cnt_tp, fp=cnt_fp);
message("Precision for lenient evaluation: ", precision);

recall <- fun_recall(tp=cnt_tp, fn=cnt_fn);
message("Recall for lenient evaluation: ", recall);
...
message("Accuracy for lenient evaluation: ", accuracy);
